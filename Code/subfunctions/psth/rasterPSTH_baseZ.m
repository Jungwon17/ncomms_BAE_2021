function [xpt,ypt,spikeBin,spikeHist,spikeConv,spikeConvZ, spikeSEM,spikeConvStd] = rasterPSTH_baseZ(spikeTime, trialIndex, win, binSize, resolution,baseline, dot)
...
if isempty(spikeTime) || isempty(trialIndex) || length(spikeTime) ~= size(trialIndex,1) || length(win) ~= 2
    xpt = []; ypt = []; spikeBin = []; spikeHist = []; spikeConv = []; spikeConvZ = []; spikeSEM = []; spikeConvStd = [];
    return;
end;
...
if sum(sum(trialIndex))>nTrial;
    nTrial = sum(sum(trialIndex));
end
yTemp = [0:nTrial-1; 1:nTrial; NaN(1,nTrial)]; % template for ypt
[xpt, ypt] = deal(cell(1,nCue));
[spikeHist, spikeConv,spikeConvStd] = deal(zeros(nCue,nSpikeBin));
...
for iCue = 1:nCue
    ...
    spkconv_std = conv(std(spkTemp,[],1),fspecial('Gaussian',[1 5*resolution],resolution),'same');
    ...
    spikeConvStd(iCue,:) = spkconv_std;
end

totalHist = histc(cell2mat(spikeTime),spikeBin)/(binSize/10^3*nTrial);
fireMean = mean(totalHist(baseline));
fireStd = std(totalHist(baseline));
