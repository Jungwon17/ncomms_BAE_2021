function h = MyScatterBarPlot(y,x,barWidth,yLim,faceColor,edgeColor,sigGroup)
...
for iGroup = 1:nGroup
    ...
    xPoint = group(iGroup) + 0.75*barWidth*(rand(nPoint,1)-0.5);
    ...
    h(iGroup).bar = bar(group(iGroup),yMean,'FaceColor',faceColor{iGroup},'EdgeColor',...
        edgeColor{iGroup},'BarWidth',barWidth,'LineWidth',0.5);
    h(iGroup).errorbar = errorbar(group(iGroup),yMean,ySem,'LineWidth',2,'Color',edgeColor{iGroup});
    errorbarT(h(iGroup).errorbar, 0.3, 0.5);

%     h(iGroup).errorbar.CapSize = barWidth*4;
    
    yPoint(yPoint>yLim(end)) = yLim(end);
    yPoint(yPoint<yLim(1)) = yLim(1);
    if length(xPoint)<=100
        if length(xPoint)<=50
            msize = 1.5;
        else
            msize = 1.5;
        end
        plot(xPoint,yPoint,'LineStyle','none','Marker','o','MarkerSize',msize,...
        'MarkerFaceColor',edgeColor{iGroup},'MarkerEdgeColor','none');
    else
        msize = 2;
%         if length(xPoint)<=200
%             msize = 2;
%         else
%             msize = 1;
%         end
        plot(xPoint,yPoint,'LineStyle','none','Marker','.','MarkerSize',msize,...
        'Color',edgeColor{iGroup});
    end
