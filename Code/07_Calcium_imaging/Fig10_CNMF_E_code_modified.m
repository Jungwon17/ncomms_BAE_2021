...
pars_envs = struct('memory_size_to_use', 110, ...   % GB, memory space you allow to use in MATLAB
    'memory_size_per_patch', 110, ...   % GB, space for loading data within one patch
...
% delete(gcp('nocreate'));
% parpool('local',early_worker); % 201111
...
if choose_params
    ...
    savefig([(fileparts(neuron.file)) '\1']); %191226
else
    [Cn1, PNR1] = neuron.correlation_pnr_parallel([1,5000]);
    set_fig = set_parameters_fig(gSiz, min_corr, min_pnr, Cn1, PNR1);
    savefig([(fileparts(neuron.file)) '\1']); %200805
end

% savefig([fileparts(fileparts(neuron.file)) '\cnmfe\1']); %191226
...
% delete(gcp('nocreate'));
% parpool('local',late_worker); % 201111
...
orderROIs_position(neuron);
cnmfe_path = neuron.save_workspace();
savefig([(fileparts(neuron.file)) '\2']); %191226

%% show neuron contours
Coor = neuron.show_contours(0.6);

savefig([(fileparts(neuron.file)) '\3']); %191226
